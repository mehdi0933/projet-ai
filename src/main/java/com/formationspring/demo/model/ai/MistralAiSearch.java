...
public class MistralAiSearch extends AbstractAiSearch {
    ...
    @Override
    public String callApi(AiDto.PostInput input) {
    ...
    """.formatted(promptMsg);
        ...
        try {
            ...
            if (root.has("choices") && root.path("choices").size() > 0) {
                ...
                aiResponse = "Erreur OpenRouter : " + root.path("error").asText();
            } else {
                aiResponse = "Réponse inattendue : " + aiResponse;
            }
            ...
            e.printStackTrace();
            return "Erreur API MISTRAL : " + e.getMessage();
        }
    }
